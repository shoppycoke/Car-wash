(function () {
  ...
  function render(prof, prestations, reductions, parrainages) {
    headerName.textContent = `Bonjour, ${prof.prenom}`;
    ...
    referralCode.textContent      = prof.code_parrainage;

    accountInfo.innerHTML = `
      <p><strong>Prénom :</strong> ${esc(prof.prenom)}</p>
      ${prof.telephone ? `<p><strong>Téléphone :</strong> ${esc(prof.telephone)}</p>` : ''}
      <p><strong>Membre depuis :</strong> ${fmtDate(prof.created_at)}</p>
      ${prof.rgpd_accepted_at ? `<p><strong>Consentement RGPD :</strong> ${fmtDate(prof.rgpd_accepted_at)}</p>` : ''}
    `;
  ...
  redeemBtn.addEventListener('click', () => {
    ...
    openModal(
      'Échanger des points',
      `Voulez-vous échanger 100 points contre 5 € de réduction ? Il vous restera ${profile.points - 100} point(s).`,
      redeemPoints
    );
  });
